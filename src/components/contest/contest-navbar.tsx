...
import { ArrowLeft, Info, BarChart2, ListOrdered, UserPlus } from "lucide-react";
...
export default function ContestNavbar({ contest, activeTab, onTabChange }: ContestNavbarProps) {
    return (
        <nav className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-white/20 dark:border-slate-700/50 sticky top-0 z-40">
            <div className="container mx-auto px-4">
                <div className="flex items-center justify-between h-16">
                    <Link href="/contests">
                        <Button variant="ghost" size="sm" className="gap-2 text-slate-600 hover:text-green-600 dark:text-slate-400 dark:hover:text-emerald-400">
                    ...
                    <div className="hidden md:flex flex-col text-right">
                        <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 line-clamp-1">{contest.name}</span>
                        <span className="text-[11px] text-slate-500 dark:text-slate-400">ID: {contest.id}</span>
                    </div>
