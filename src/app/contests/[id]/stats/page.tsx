...
export default function ContestStatsPage() {
    ...
    const stats = {
        ...
        problemStats: [
            { problem: "A", title: "Tìm số lớn nhất", solved: 2847, attempted: 3241, difficulty: "Dễ", color: "bg-green-500" },
            { problem: "B", title: "Đếm số cặp", solved: 1932, attempted: 2876, difficulty: "Dễ", color: "bg-green-500" },
            { problem: "C", title: "Dãy con tăng dài nhất", solved: 876, attempted: 2156, difficulty: "Trung bình", color: "bg-yellow-500" },
            { problem: "D", title: "Cây khung nhỏ nhất", solved: 543, attempted: 1892, difficulty: "Trung bình", color: "bg-yellow-500" },
            { problem: "E", title: "Đường đi ngắn nhất", solved: 234, attempted: 1456, difficulty: "Khó", color: "bg-red-500" },
            { problem: "F", title: "Thuật toán động", solved: 123, attempted: 987, difficulty: "Khó", color: "bg-red-500" },
            { problem: "G", title: "Cấu trúc dữ liệu", solved: 67, attempted: 543, difficulty: "Rất khó", color: "bg-gray-800" },
            { problem: "H", title: "Lý thuyết số", solved: 23, attempted: 234, difficulty: "Rất khó", color: "bg-gray-800" },
        ]
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-slate-700/50 shadow-xl p-6 hover:shadow-2xl transition-all duration-300 group">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-sm text-slate-500 dark:text-slate-400">Tổng bài nộp</p>
                                <p className="text-2xl font-bold text-slate-800 dark:text-slate-100">{stats.submissions.toLocaleString()}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-slate-700/50 shadow-xl p-6 hover:shadow-2xl transition-all duration-300 group">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-sm text-slate-500 dark:text-slate-400">Bài đã AC</p>
                                <p className="text-2xl font-bold text-slate-800 dark:text-slate-100">{stats.solvedProblems.toLocaleString()}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-slate-700/50 shadow-xl p-6 hover:shadow-2xl transition-all duration-300 group">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-slate-800 dark:text-slate-100">{stats.averageTime}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-slate-700/50 shadow-xl p-6 hover:shadow-2xl transition-all duration-300 group">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-slate-800 dark:text-slate-100">{stats.topScore}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-slate-700/50 shadow-xl p-8">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100">Ngôn ngữ lập trình</h3>
                        </div>
                    ...
                    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-slate-700/50 shadow-xl p-8">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100">Thống kê theo bài</h3>
                        </div>
                        <div className="space-y-4 max-h-80 overflow-y-auto">
                            {stats.problemStats.map((problem, index) => {
                                const solveRate = Math.round((problem.solved / problem.attempted) * 100);
                                return (
                                    <div key={index} className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`px-2 py-1 rounded text-xs font-medium text-white ${problem.color}`}>
                                                        {problem.difficulty}
                                                    </span>
                                                </div>
                                                <p className="text-sm text-slate-600 dark:text-slate-300">{problem.title}</p>
                                            </div>
